...
| CONFIG-004 | P1 | ✅ | Time Zone | See correct timestamps when the server runs outside UTC | CMS process in a non-UTC zone (E2E pins `Europe/Prague`) | Sign in from a second device, Account → Active devices | The fresh session reads "Just now" and the sessions API returns ISO 8601 UTC timestamps within the last minute | zone-offset drift on SQLite `current_timestamp` rows, raw `YYYY-MM-DD HH:MM:SS` strings reaching the browser, mixed timestamp shapes in one column |
