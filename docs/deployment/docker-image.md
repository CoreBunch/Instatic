...
docker pull ghcr.io/corebunch/instatic:0.0.15
```

The v0.0.15 published image is built for `linux/amd64`. Use it on Railway and x86_64 VPS/container hosts. ARM64 hosts should build from source for now, or wait for the native arm64 release job before pulling GHCR images directly.
...
```txt
ghcr.io/corebunch/instatic:0.0.15
```
...
Enable Railway Image Auto Updates when you want Railway to move the service forward automatically during a maintenance window. Use `:latest` for "always follow the newest image", or a semver tag such as `:0.0.15` if you want Railway's semver update controls.
