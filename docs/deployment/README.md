...
This index maps the two Instatic deployment boundaries to the files, variables, and persistence rules they need.

The **builder workspace** uses the root `Dockerfile`. It serves the editor, CMS API, publisher, and optionally the public site from one Bun process backed by SQLite or Postgres. A **website-only artifact** uses the Dockerfile generated by `bun run site:export`. It serves only published files and has no database, admin, auth, or builder source. See [site artifacts](../features/site-artifacts.md).
...
|---|---|---|---|---|
| Railway website artifact | Host only the site produced by Instatic | None | None; redeploy immutable artifacts | [railway.md](railway.md) |
...
Builder workspaces must back up both the database and uploaded media. See [backup-restore.md](backup-restore.md). Website-only artifacts are reproducible outputs; archive them when release rollback is required.

## Website Runtime Contract

The exported website runtime needs only `PORT` (default `3000`). Its Docker context contains `runtime.ts` and `public/`; `railway.json` configures `/health`. It needs no `DATABASE_URL`, `UPLOADS_DIR`, `STATIC_DIR`, secret key, or volume.

## Builder Runtime Contract

Every builder deployment target configures the full workspace process:
