...
Selection rings and hover rings are absolutely-positioned overlay divs portaled into the canvas root—outside the iframe and the transform layer. The selection ring is a 1px `box-shadow` using `--canvas-selection-ring`; the hover ring is a 1px dashed border in `--canvas-hover-ring-color` (60% accent) so "could select" reads differently from "is selected". Both derive from the single `--canvas-accent` token. Because the rings live in the canvas root's coordinate space rather than inside the scaled transform layer, that 1px border stays exactly 1px at every zoom level. Single-selection in the active frame additionally grows **eight resize handles** (corners + edge midpoints; with the inspector's Size ratio lock on — session state `sizeRatioLocked` — a drag keeps the element's proportions: an edge handle derives the other axis, a corner follows the axis the pointer moved more; Shift flips the lock for one drag), **four corner-radius dots** (round, just inside each corner on its diagonal, `radius × zoom` from the corner — pull inward to round; which corners move follows the inspector's Radius row scope, session state `radiusScope` in `selectionSlice`: "all corners" rounds all four, "separately" only the grabbed one, Alt flips it for the drag; writes the `border*Radius` longhands the Radius row uses and clears a stored `borderRadius` shorthand) and a **dimension badge** (`700 × 412`, the element's CSS-px size). Every canvas style gesture — resize, radius — rides one skeleton, `canvasStyleGesture.ts`: moves preview via inline style in the iframe (with a throttled echo into `canvasGesturePreview` so the inspector follows live), release commits ONE style write through the active style target, Escape cancels, and a non-base breakpoint without a class refuses with a toast instead of leaking into the base inline styles. Free-move drags snap (±4 screen px) to sibling / parent edges and centres, drawing accent smart-guide hairlines.
...
| `<style data-instatic-canvas-chrome>` | `iframeBodyReset.ts` (design frames only) | **unlayered**, `!important` | Neutralises interaction affordances so the frame reads as click-to-select: `cursor`, `user-select`, focus `outline`, nested-iframe pointer events — and the site's `transition`s on `[data-node-id]` and descendants, so every edit lands the same frame instead of gliding after the cursor (animations are left alone). Consequence: the `cursor`, `userSelect` and `transition` rows show no effect on the canvas; they apply on the published page and in the live frame |
| `<style id="mc-classes">` | `ClassStyleInjector` | `@layer user-authored` | Publisher reset + framework CSS + class registry CSS |
| `<style id="mc-classes-preview">` | `ClassStyleInjector` | `@layer user-authored` | Higher-specificity preview rule while a property control is hovered or typed into (`previewClassStyles`); empty for state-pseudo rules. Inline-style targets preview through the sibling channel `previewInlineStyles`, which `NodeRenderer` overlays on the node's stored bag — same keystroke-level liveness, no stylesheet involved |
