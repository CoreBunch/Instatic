...
Instatic has three architectural boundaries. The managed platform owns agency accounts and project metadata; a builder workspace owns the private editor and one site's mutable CMS state; a website deployment owns only the exported artifact and generic runtime. The visual editor's output is plain semantic HTML and hand-clean CSS — no framework runtime is injected into published pages.

---

## TL;DR

- **One builder process, two worker families**: `bun server/index.ts`. `Bun.serve` + a hand-written router (`server/router.ts`) routes every workspace request. Plugin server code runs in one `Bun.Worker` per active plugin, each wrapping a QuickJS-WASM sandbox; image-variant generation (`sharp` + BlurHash) runs in a separate `Bun.Worker` pool. Everything else — HTTP, the admin API, the streaming agent endpoint, the publisher — runs on the main thread.
- **One-way site deployment boundary**: a full publish emits a schema-versioned manifest, pre-rendered routes, browser assets, dependency cache, and media inventory. `bun run site:export` materializes those files with `site-runtime/`, a generic Bun server that has no database or builder imports. See [site artifacts](features/site-artifacts.md).
- **A separate agency control plane**: `/app` is a distinct Vite entry and `/app/api/*` uses a separate database selected by `CONTROL_PLANE_DATABASE_URL`. WorkOS AuthKit supplies production identity and organization context; local development uses an explicit development identity. See [managed platform](features/managed-platform.md).
- **Two database domains, two engines**: the CMS database is selected by `DATABASE_URL`; the platform database is selected independently by `CONTROL_PLANE_DATABASE_URL`. Each can use Postgres or SQLite. Platform tables and migrations live under `server/platform/` and never enter a website export.
...
- **Two private frontends, separate entries**: the project dashboard (`src/platform/`) is built for `/app/*`. The CMS admin (`src/admin/`) shells the visual editor (`src/admin/pages/site/`) at `/admin/*`. They share build tooling and selected UI primitives, but not application state or routing.
...
In combined mode, the builder process serves visitors, admins, the API, the streaming agent endpoint, and uploads. In separated mode, visitors reach `site-runtime/runtime.ts` and the builder process is not part of the public deployment. Two kinds of builder work that would otherwise block the main thread are pushed into `Bun.Worker`s:
...
| Static artefact IO           | `server/publish/staticArtefact.ts`    | Layer A: cross-platform two-slot pointer swap, atomic per-file rename, slot-aware read/write/purge. |
...
publishDraftSite / publishDataRow      ← server/publish/publishSite.ts / publishRow.ts
    ...
    │  5. swapSlot — atomic current-pointer flip (symlink or marker)
...
- **Atomic publishing.** Full publishes build the inactive `a/` or `b/` slot, then atomically replace the `current` pointer. POSIX uses a relative directory symlink; Windows uses a one-byte marker file so local publishing needs neither Developer Mode nor elevated symlink privileges. Readers retry against the latest pointer during a swap. Incremental row publish (`publishDataRow`) writes a single file via tmp + rename into the active slot.
...
- Source-of-truth files:
  ...
  - `server/publish/staticArtefact.ts` — Layer A cross-platform two-slot pointer swap
