...
- **Provider-agnostic.** The runtime selects a driver (Anthropic, OpenAI, OpenRouter, Ollama, OrcaRouter, Custom Provider) from the conversation's configured credential.
...
│   └── orcarouter.ts          — OrcaRouter driver: OpenAI-compatible gateway; apiKey auth; live GET /v1/models catalogue
└── runtime/
...
| `orcarouter` | OrcaRouter | `apiKey` | API key (`sk-orca-…`) | — | Live `GET /v1/models` (standard OpenAI list shape); models enriched with OpenRouter prices + context windows |

**OrcaRouter** (id `orcarouter`) is a named OpenAI-compatible gateway at `https://api.orcarouter.ai/v1` that fronts frontier models from many providers behind one key. It reuses the shared chat/completions adapter, so capabilities default to `{ toolCalling: true, visionInput: false, toolResultImages: false, promptCache: false, streaming: true }`, and model ids are the gateway's OpenRouter-style slugs (`openai/gpt-5.6-sol`, `anthropic/claude-sonnet-5`, …). The live `/v1/models` catalogue is enriched from the OpenRouter price catalogue the same way as Anthropic/OpenAI, since the models are the same upstream rows.
