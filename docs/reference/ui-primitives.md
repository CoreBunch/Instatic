...
| `ColorInput`       | Colour swatch trigger; opens the `ColorPicker` in a draggable floating panel (opens left of the trigger; ×/Escape/outside click closes, so at most one is open) | `value`, `swatchValue?`, `onValueChange`, `gradients?`, `tokens?`, `onSelectToken?`, `onCreateToken?` |
| `ColorPicker`      | Rich colour surface: saturation/brightness square, hue + alpha sliders, HEX/RGB/HSL field + opacity %, native `EyeDropper` (feature-detected), searchable token list, "New Style"; with `gradients` also Solid/Linear/Radial/Conic fill tabs + a stop strip that emits CSS gradient strings; numeric fields (opacity, angle) carry their unit in the value with a hover-revealed stepper | `value`, `onChange`, `gradients?`, `tokens?`, `onSelectToken?`, `onCreateToken?` |
