...
| `SegmentedControl` | A few mutually exclusive options shown inline; `value` can be `undefined` for an unset state where no segment appears pressed. Two looks: `track` (default — padded rail, one floating active pill) and `tiles` (inspector redesign — fused surface tiles split by 1px seams, content-width and right-aligned unless `fullWidth`; used by the Layout/Position switcher family) | `options`, `value`, `onChange`, `look?: 'track' \| 'tiles'`, `onClear?` (deselectable — clicking the active segment fires `onClear` and shows a hover close-icon overlay) |
...
| Primitive          | When to use                                                          | Key props                                                  |
|--------------------|----------------------------------------------------------------------|------------------------------------------------------------|
| `Input`            | Single-line text input. Borderless field fill (`--bg-surface-2` on `--bg-surface`, hover lifts to `--bg-surface-3`, border appears on focus), `--control-radius` corners; heights ride the `--control-height*` scale (`fieldSize`: xs=24 / sm=30 / md=36 — the shared `FieldSize` type from `@ui/fieldSize`) | `value`, `onChange`, `placeholder`, `type`, `error`, `fieldSize` |
...
| `ColorInput`       | Colour swatch trigger; opens the `ColorPicker` in a draggable floating panel (opens left of the trigger; ×/Escape/outside click closes — except on `[data-color-picker-keep-open]` companion surfaces like the canvas gradient gizmo — so at most one is open); `onOpenChange` reports open/close so the editor can gate canvas affordances | `value`, `swatchValue?`, `onValueChange`, `onOpenChange?`, `gradients?`, `tokens?`, `onSelectToken?`, `onCreateToken?` |
| `ColorPicker`      | Rich colour surface: saturation/brightness square, hue + alpha sliders, HEX/RGB/HSL notation switch — a text field for HEX, one stepper per channel for RGB (R/G/B) and HSL (H/S/L), plus an alpha field in every notation, native `EyeDropper` (feature-detected), searchable token list, "New Style"; with `gradients` also Solid/Linear/Radial/Conic fill tabs + a stop strip that emits CSS gradient strings; numeric fields (opacity, angle) carry their unit in the value with a hover-revealed stepper | `value`, `onChange`, `gradients?`, `tokens?`, `onSelectToken?`, `onCreateToken?` |
...
| `Section`          | Collapsible titled section inside a panel. Header anatomy: rotating caret · title · mono count (`meta`), with `headerAction` hugging the right edge; the whole header is the toggle | `title`, `children`, `defaultOpen`, `icon`, `meta`, `indicator`, `forceOpen`, `flush`, `headerAction` |
| `ControlRow`       | Label + control row in property panels. 100px label column (`--control-label-column`); `narrow` switches to the popout interior's 52px variant. Rows centre on `--control-height` | `label`, `description`, `narrow`, `children`               |
