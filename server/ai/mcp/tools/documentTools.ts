...
import { describeAgentDocuments, toolLocaleId } from '@core/ai'
...
export const documentMcpTools: AiTool[] = [
  {
    ...
    scope: 'site',
    execution: 'server',
    inputSchema: Type.Object({ localeId: Type.Optional(Type.String({ minLength: 1 })) }, { additionalProperties: false }),
    requiredCapabilities: ['site.read'],
    handler: async (input, ctx: ToolContext) => {
      const site = await getDraftSiteDocument(ctx.db, { localeId: toolLocaleId(input, ctx.snapshot) })
