...
const PLUGIN_ROUTES: readonly Route<[CmsHandlerOptions, AuthUser]>[] = [
  ...
  { method: 'PATCH', pattern: PLUGIN_RECORD_ITEM_PATTERN, handler: (req, db, p, _o, user) => handlePluginRecordItem(req, db, user, p.id, p.rid, p.rec) },
  { method: 'DELETE', pattern: PLUGIN_RECORD_ITEM_PATTERN, handler: (req, db, p, _o, user) => handlePluginRecordItem(req, db, user, p.id, p.rid, p.rec) },
  { method: 'GET', pattern: PLUGIN_RECORDS_PATTERN, handler: (req, db, p, _o, user) => handlePluginRecordsCollection(req, db, user, p.id, p.rid) },
  { method: 'POST', pattern: PLUGIN_RECORDS_PATTERN, handler: (req, db, p, _o, user) => handlePluginRecordsCollection(req, db, user, p.id, p.rid) },
