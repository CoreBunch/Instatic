...
A CMS and visual website builder with a first-class plugin system. The builder workspace runs on Bun and is backed by either Postgres or SQLite (selected by `DATABASE_URL`). Its output is intentionally plain, semantic HTML with hand-clean CSS — no framework runtimes injected into published pages.

The managed platform, builder workspace, and published website are separate architectural boundaries. `/app` owns agency/project metadata and WorkOS sessions; `/admin` is the private single-site builder; a website-only deployment contains a versioned site artifact plus the generic `site-runtime/` shell. The site deployment must never contain the platform, admin, CMS APIs, database layer, auth, AI providers, plugin installer, or builder source. These boundaries are gated by `platform-boundary.test.ts` and `site-runtime-isolation.test.ts`. See [`docs/features/managed-platform.md`](docs/features/managed-platform.md) and [`docs/features/site-artifacts.md`](docs/features/site-artifacts.md).
...
- **Publishing:** Three-layer pipeline. **Layer A** bakes fully-static pages to the active `uploads/published/{a,b}` slot and atomically replaces the `current` pointer (POSIX symlink, Windows marker file; `server/publish/staticArtefact.ts`). **Layer B** is an in-memory LRU keyed by `(urlPath, queryString, publishVersion)` for dynamic routes (`server/publish/renderCache.ts`); `bumpPublishVersion()` evicts wholesale on every publish. **Layer C** emits `<instatic-hole>` placeholders for nodes auto-detected as request-dependent; a ~668 B `IntersectionObserver` runtime lazy-fetches each fragment from `/_instatic/hole/<nodeId>`. Auto-detection lives in `src/core/publisher/dynamicDetection.ts` — one walker, four rules. Every full publish also writes `/.instatic/site-artifact.json`; portable artifacts can be exported with `bun run site:export` and served by the builder-free `site-runtime/`. Full design: [`docs/features/publisher.md`](docs/features/publisher.md) and [`docs/features/site-artifacts.md`](docs/features/site-artifacts.md).
...
site-runtime/   Generic, database-free runtime copied into website-only deployments
